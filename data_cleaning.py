...
# read the csv file
with open(fname, 'r', encoding="utf-8") as in_file, open("cleaned_data.csv", 'w', encoding="utf-8", newline='') as out_file:
    ...
    for row in rows:
        print("row: " + str(loop_counter), end="\r")
        loop_counter += 1
        if len(row) >= 3:
            all_counter += 1
    ...
    print("Processing row: " + str(loop_counter))
